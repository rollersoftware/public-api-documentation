...
The [**Tickets**](../reference/reporting-api.yaml/paths/~1data~1tickets/get) endpoint response includes `ProductId`, which can be used to quantify the number of visits for a specific product if required. You can do this by associating the `BookingSignedWaivers` with tickets that have a particular `ProductId`.
