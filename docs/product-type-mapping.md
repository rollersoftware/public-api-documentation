...
The `ProductType` and `ProductSubType` properties (for example, those returned via [**Get products**](https://docs.roller.app/docs/api/reporting/operations/get-products)) correspond to a type of product in ROLLER. The table below maps the property values to expect for each corresponding ROLLER product type.
