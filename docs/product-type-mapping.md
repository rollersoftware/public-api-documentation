...
The `ProductType` and `ProductSubType` properties (for example, those returned via [**Get products**](../reference/data-api.yaml#/paths/~1data~1products/get)) correspond to a type of product in ROLLER. The table below maps the property values to expect for each corresponding ROLLER product type.
