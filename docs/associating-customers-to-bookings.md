...
Within the [**Booking items**](../reference/reporting-api.yaml/paths/~1data~1bookingitems/get) endpoint, there is a `BookingCustomerId` field, which is the `CustomerId` of the booking owner. If a booking owner is assigned, then the spend on that booking can be attributed directly to that customer.
