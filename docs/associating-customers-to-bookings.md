...
Within the [**Booking items**](../reference/data-api.yaml) endpoint, there is a `BookingCustomerId` field, which is the `CustomerId` of the booking owner. If a booking owner is assigned, then the spend on that booking can be attributed directly to that customer.
